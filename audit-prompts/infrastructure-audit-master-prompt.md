...
1. **Tracking issue first** — `[AUDIT] Infrastructure — Befund-Tracker & Roadmap`. Body: a
   management summary (verdict, grade, biggest outage/breach risk), the scorecard, a
   **priority-sorted checklist** (P0→P3, then effort/priority) where each line links its child
   issue, and the 30/60/90 roadmap. Labels: `audit`, `tracking`, `infra`.
2. **One issue per confirmed finding** — top-notch, German, each opening with its own
   **management summary** (2–3 sentences: what, impact, one-line recommendation), then the full
   finding (severity + score, standard mapping, resources, evidence, impact, concrete
